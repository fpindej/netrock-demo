    ...
    IUserContext userContext) : ApiController
{
    ...
    /// <returns>A paginated list of contacts ordered by favorite status and creation date.</returns>
        ...
        CancellationToken cancellationToken)
    {
        ...
        var result = await contactsService.GetContactsAsync(userId, request.PageNumber, request.PageSize, cancellationToken);
        ...
        return Ok(new ListContactsResponse
        {
    ...
    /// </summary>
    /// <param name="request">The bulk delete request containing contact IDs.</param>
    /// <param name="cancellationToken">A cancellation token.</param>
    /// <returns>The number of deleted contacts.</returns>
    /// <response code="200">Returns the count of deleted contacts.</response>
    /// <response code="400">If the request is invalid.</response>
    /// <response code="401">If the user is not authenticated.</response>
    [HttpDelete("bulk")]
    ...
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    [ProducesResponseType(StatusCodes.Status401Unauthorized)]
    public async Task<ActionResult<int>> BulkDeleteContacts(
        [FromBody] BulkDeleteContactsRequest request,
        CancellationToken cancellationToken)
    {
        var userId = userContext.AuthenticatedUserId;
    ...
    /// <param name="cancellationToken">A cancellation token.</param>
    /// <returns>The updated contact.</returns>
    /// <response code="200">Contact favorite status toggled.</response>
    /// <response code="401">If the user is not authenticated.</response>
    /// <response code="404">If the contact was not found.</response>
    ...
    [ProducesResponseType(StatusCodes.Status401Unauthorized)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task<ActionResult<ContactResponse>> ToggleFavorite(
        Guid id,
        CancellationToken cancellationToken)
    {
        var userId = userContext.AuthenticatedUserId;
        var result = await contactsService.ToggleFavoriteAsync(userId, id, cancellationToken);
