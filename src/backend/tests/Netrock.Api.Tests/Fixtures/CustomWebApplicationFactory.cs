...
using IAuthenticationService = Netrock.Application.Features.Authentication.IAuthenticationService;

namespace Netrock.Api.Tests.Fixtures;
...
public class CustomWebApplicationFactory : WebApplicationFactory<Program>
{
    ...
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        ...
        builder.ConfigureTestServices(services =>
        {
            ...
            services.RemoveAll<DbContextOptions<NetrockDbContext>>();
            services.RemoveAll<DbContextOptions>();
            services.RemoveAll<NetrockDbContext>();

            // Manually register InMemory options (bypasses AddDbContext's TryAdd)
            services.AddScoped(_ =>
            {
                var optionsBuilder = new DbContextOptionsBuilder<NetrockDbContext>();
                optionsBuilder.UseInMemoryDatabase(_dbName);
                return optionsBuilder.Options;
            });

            services.AddScoped<DbContextOptions>(sp =>
                sp.GetRequiredService<DbContextOptions<NetrockDbContext>>());

            services.AddScoped<NetrockDbContext>();
