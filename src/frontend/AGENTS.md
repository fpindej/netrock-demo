...
| Setting                         | Effect                                                      |
| ------------------------------- | ----------------------------------------------------------- |
| `engine-strict=true`            | Fails install if Node version doesn't match `engines` field |
| `strict-peer-dependencies=true` | Fails on unmet peer deps instead of just warning            |
| `frozen-lockfile=true`          | Enforces lockfile integrity everywhere (dev, CI, Docker)    |

Both `Dockerfile` and `Dockerfile.local` copy `.npmrc` into the image so these settings apply in Docker builds.
