...
1. Add a constant to `src/backend/Netrock.WebApi/Shared/RateLimitPolicies.cs`:
   ...
   ```
2. Add a nested configuration class inside `RateLimitingOptions` in `src/backend/Netrock.WebApi/Options/RateLimitingOptions.cs` (extend `FixedWindowPolicyOptions`):
   ```csharp
...
**1. Create the model record** in `src/backend/Netrock.Application/Features/Email/Models/EmailTemplateModels.cs`:
...
**2. Create the 3 template files** in `src/backend/Netrock.Infrastructure/Features/Email/Templates/`:
...
**3. Register the model type** in `FluidEmailTemplateRenderer.CreateOptions()` in `src/backend/Netrock.Infrastructure/Features/Email/Services/FluidEmailTemplateRenderer.cs`:
...
**5. Add tests** in `src/backend/tests/Netrock.Component.Tests/Services/FluidEmailTemplateRendererTests.cs` — at minimum test subject rendering, HTML variable injection, and plain text variant.

**6. Verify:** `dotnet test src/backend/Netrock.slnx -c Release`
